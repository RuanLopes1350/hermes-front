...
import { CheckCircle2, Clock, AlertCircle, Activity, TrendingUp, ArrowUpRight } from 'lucide-react';
		...
		<div className="space-y-10 text-left">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				{stats.map((stat, i) => (
					<Card
						key={i}
						className="bg-surface border-border-subtle rounded-2xl p-6 hover:border-primary/30 transition-all group duration-300 shadow-sm border"
					>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
				...
				<Card className="lg:col-span-2 bg-surface border-border-subtle rounded-2xl p-8 min-h-[400px] flex flex-col border text-left">
					<div className="flex justify-between items-center mb-8">
						...
						<Link href="/system/logs">
							<Button
								variant="link"
								className="text-[10px] font-black uppercase tracking-widest text-primary gap-1 p-0 h-auto"
							>
					...
					<div className="flex-1 border border-dashed border-border-subtle rounded-3xl flex items-center justify-center bg-background/50 group cursor-default">
						<div className="text-center">
							<TrendingUp
								size={48}
								className="mx-auto mb-4 text-muted-foreground/20 group-hover:text-primary/40 transition-colors duration-500"
							/>
				...
				<Card className="bg-surface border-border-subtle rounded-2xl p-8 flex flex-col border text-left">
					<CardTitle className="text-lg font-bold mb-6 uppercase tracking-tight text-foreground">
						Status do Sistema
					</CardTitle>
					...
					<div className="space-y-6">
						...
						<div className="space-y-4 pt-4">
							{[
								...
								<div
									key={i}
									className="flex items-center justify-between p-4 rounded-2xl bg-background/40 border border-border-subtle/50"
								>
			...
			<Card className="bg-surface border-border-subtle rounded-3xl overflow-hidden shadow-sm border text-left">
				<div className="p-8 border-b border-border-subtle flex justify-between items-center bg-background/30">
					...
					<Link href="/system/logs">
						<Button
							variant="outline"
							className="px-5 py-2.5 rounded-xl bg-background text-[10px] font-black uppercase text-muted-foreground border-border-subtle hover:text-primary hover:border-primary/50 transition-all tracking-widest"
						>
							Ver Tudo
						</Button>
					</Link>
					...
					<Table>
						<TableHeader className="bg-background/50">
							<TableRow className="border-b border-border-subtle/30">
								<TableHead className="px-8 py-5 text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-bold">
									Data/Hora
								</TableHead>
								<TableHead className="px-8 py-5 text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-bold">
									Serviço
								...
								<TableHead className="px-8 py-5 text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-bold text-right">
									Duração
								</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							{[
								...
								<TableRow
									key={i}
									className="hover:bg-white/5 transition-colors cursor-pointer group border-b border-border-subtle/30"
								>
									<TableCell className="px-8 py-6 text-muted-foreground text-xs italic font-medium text-left">
										{log.time}
									</TableCell>
									<TableCell className="px-8 py-6 font-bold text-foreground uppercase tracking-tight text-left">
										{log.service}
									</TableCell>
									<TableCell className="px-8 py-6 text-xs font-mono text-primary/80 text-left">
										{log.event}
									</TableCell>
									<TableCell className="px-8 py-6 text-left">
										<Badge
											variant="outline"
											className={
												'border-none flex items-center gap-1.5 ' +
												log.color +
												' font-black text-[9px] uppercase tracking-widest bg-transparent p-0'
											}
										>
											<span
												className={'w-1.5 h-1.5 rounded-full ' + log.color.replace('text', 'bg')}
											></span>
											{log.status}
										</Badge>
									</TableCell>
									<TableCell className="px-8 py-6 text-muted-foreground text-xs font-mono text-right">
										{log.duration}
									</TableCell>
								</TableRow>
							))}
