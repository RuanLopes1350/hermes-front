    ...
    const handleDeleteUser = async (id: string, name: string) => {
        if (id === session?.user.id) {
            toast({
                ...
                description: "Por segurança, você não pode remover sua própria conta administrativa por aqui.",
            });
            return;
        }

        if (!confirm(`Confirmar exclusão de "${name}"? Esta ação removerá permanentemente o acesso deste usuário.`)) {
            return;
        }
                    ...
                    <Table>
                        <TableHeader className="bg-background/50">
                            <TableRow className="border-b border-border-subtle/30">
                                <TableHead className="px-8 py-5 text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Identificação</TableHead>
                                <TableHead className="px-8 py-5 text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Permissões</TableHead>
                                <TableHead className="px-8 py-5 text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Membro desde</TableHead>
                            ...
                            {loading && users.length === 0 ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={4} className="h-48 text-center text-muted-foreground italic">
                                        {searchTerm ? "Nenhum resultado para sua busca." : "Nenhum usuário encontrado na base de dados."}
                                    </TableCell>
                                </TableRow>
                            ) : (
                                    ...
                                    <TableRow key={user.id} className="hover:bg-white/5 transition-colors group border-b border-border-subtle/30">
                                                ...
                                                <div className="text-left">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {user.id === session?.user.id && (
                                                            <Badge className="bg-primary/20 text-primary border-none text-[8px] h-4 px-1">VOCÊ</Badge>
                                                        )}
                                                    </div>
                                        ...
                                        <TableCell className="px-8 py-6 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                {(session?.user && (session.user as any).isAdmin) && user.id !== session?.user.id && (
                                                    <Button 
                                                        variant="ghost" 
                                                        size="icon" 
