	...
	return (
		<div className="space-y-10 text-left">
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div>
					<h2 className="text-3xl font-bold tracking-tight mb-2 uppercase text-text-primary">
						Chaves de API
					</h2>
					<p className="text-text-secondary text-sm font-medium italic">
						Gerencie os tokens de acesso vinculados às suas conexões.
					</p>
				</div>
				{/* 
                ...
                */}
				<div className="bg-primary/5 border border-primary/20 px-4 py-2 rounded-xl flex items-center gap-3">
					<p className="text-[10px] font-bold text-primary uppercase tracking-wider">
						As chaves são geradas ao criar novas conexões
					</p>
				...
				<div className="grid grid-cols-1 gap-6">
					{keys.length === 0 ? (
						...
						keys.map((key) => (
							<div
								key={key.id}
								className="group bg-surface border border-border-subtle rounded-3xl p-8 hover:border-primary/30 transition-all shadow-sm relative overflow-hidden"
							>
								<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
									{/* Info Principal */}
									<div className="flex items-start gap-6">
										<div
											className={`p-4 rounded-2xl ${key.is_active ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}
										>
											<Key size={24} />
										</div>
										<div className="space-y-1">
											...
											<div className="flex items-center gap-4 text-[10px] text-text-secondary font-medium uppercase tracking-wider italic">
												<span className="flex items-center gap-1.5">
													<Shield size={12} className="opacity-50" /> Service: {key.service_id}
												</span>
												<span className="flex items-center gap-1.5">
													<Calendar size={12} className="opacity-50" /> Criada em{' '}
			...
			<div className="bg-primary/5 border border-primary/20 p-6 rounded-3xl flex items-start gap-4">
				...
				<div className="space-y-1">
					...
					<p className="text-xs text-text-secondary italic leading-relaxed">
						O Hermes nunca armazena o token original da sua API Key. Apenas um hash criptográfico
						irreversível é salvo no banco de dados. Se você perder sua chave, precisará gerar uma
						nova conexão.
					</p>
				</div>
			</div>
