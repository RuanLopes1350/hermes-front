        ...
        <div className="space-y-12 text-left pb-20">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 text-left">
                <div className="flex items-center gap-4 text-left">
                    <Link href="/system/services"><Button variant="outline" size="icon" className="h-12 w-12 rounded-2xl bg-surface border-border-subtle hover:text-primary group"><ArrowLeft size={20} /></Button></Link>
                    <div className="text-left">
                        <div className="flex items-center gap-3 mb-1 text-left"><h2 className="text-3xl font-bold uppercase text-foreground text-left">{service?.name}</h2><Badge className="bg-success/10 text-success border-none uppercase text-[10px]">Ativo</Badge></div>
                        <p className="text-muted-foreground text-sm italic text-left">Gestão centralizada de segurança e remetentes.</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left">
                <div className="lg:col-span-2 space-y-12 text-left">
                    ...
                    <Card className="bg-surface border-border-subtle rounded-[40px] p-10 border shadow-sm text-left">
                        <CardHeader className="p-0 mb-10 flex flex-row items-center gap-3 text-left">
                            ...
                            <div className="text-left"><CardTitle className="text-xl font-bold italic uppercase tracking-tighter text-left">Ajustes do Projeto</CardTitle></div>
                        </CardHeader>
                        <CardContent className="p-0 space-y-10 text-left">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                                <div className="space-y-3 text-left"><label className="text-[10px] font-bold text-muted-foreground uppercase px-1 text-left">Nome do Serviço</label><Input value={editName} onChange={(e) => setEditName(e.target.value)} className="bg-background border-border-subtle rounded-xl px-4 py-6 text-sm focus:border-primary font-medium h-12" /></div>
                                <div className="space-y-3 text-left"><label className="text-[10px] font-bold text-muted-foreground uppercase px-1 text-left">Timezone</label><CustomSelect value={editSettings.timezone} onValueChange={(val) => setEditSettings(prev => ({ ...prev, timezone: val }))} options={[{ value: "America/Sao_Paulo", label: "Brasília" }, { value: "UTC", label: "UTC" }]} /></div>
                            </div>
                    ...
                    <Card className="bg-surface border-border-subtle rounded-[40px] p-10 border shadow-sm text-left">
                        <CardHeader className="p-0 mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 text-left">
                            <div className="flex items-center gap-3 text-left">
                                ...
                                <div className="text-left"><CardTitle className="text-xl font-bold italic uppercase tracking-tighter text-left">Acesso e Disparo</CardTitle><p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-1 text-left">Conexões SMTP com Chaves de API vinculadas</p></div>
                            </div>
                        ...
                        <CardContent className="p-0 space-y-6 text-left">
                            {unifiedConnections.length === 0 ? (
                                <div className="py-20 border border-dashed border-border-subtle rounded-[32px] text-center italic text-muted-foreground text-sm">Nenhuma conexão de segurança configurada.</div>
                            ) : (
                                <div className="grid grid-cols-1 gap-6 text-left">
                                    {unifiedConnections.map(conn => (
                                        <div key={conn.id} className="p-8 bg-background/50 rounded-[32px] border border-border-subtle hover:border-primary/30 transition-all group relative overflow-hidden text-left">
                                            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10 text-left">
                                                <div className="flex items-start gap-6 text-left">
                                                    ...
                                                    <div className="space-y-1 text-left">
                                                        <div className="flex items-center gap-3 text-left">
                                                            <h4 className="text-lg font-bold text-foreground text-left">{conn.name}</h4>
                                                            <Badge className="bg-success/10 text-success border-none text-[8px] uppercase tracking-widest">{conn.auth_type === 'oauth2' && !conn.refresh_token ? 'Aguardando Google' : 'Pronto'}</Badge>
                                                        </div>
                                                        <p className="text-[11px] text-muted-foreground italic font-medium text-left">{conn.login}</p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex flex-col lg:items-end gap-3 text-left">
                                                    <div className="flex items-center gap-3 bg-background p-2.5 pl-5 rounded-2xl border border-border-subtle w-full max-w-sm group/key hover:border-primary/40 transition-colors text-left">
                                                        <div className="flex items-center gap-2 flex-1 text-left">
                                                            ...
                                                            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-left">API KEY:</span>
                                                            <code className="text-xs font-mono text-foreground font-bold text-left">
                                                                {conn.key ? `${conn.key.prefix}••••••••` : 'NÃO GERADA'}
                                                            </code>
                                                        </div>
                                                        <Button variant="ghost" size="icon" onClick={() => confirmDeleteConnection(conn.id, conn.name)} className="h-9 w-9 text-muted-foreground hover:text-danger hover:bg-danger/10 rounded-xl"><Trash2 size={18} /></Button>
                                                    </div>
                                                    <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-tighter px-2 text-left">A chave é revogada automaticamente ao excluir o remetente.</p>
                                                </div>
                                            </div>
                                            <div className="absolute -right-10 -bottom-10 opacity-[0.02] group-hover:opacity-[0.05] transition-opacity">
                                                {conn.auth_type === 'oauth2' ? <FaGoogle size={120} /> : <Shield size={120} />}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                ...
                <div className="space-y-8 text-left">
                    <Card className="bg-surface border-border-subtle rounded-3xl p-8 border shadow-sm text-left">
                        <CardHeader className="p-0 text-left"><CardTitle className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2 text-left"><Code size={14} className="text-primary" /> Endpoints</CardTitle></CardHeader>
                        <CardContent className="p-0 mt-6 text-left"><p className="text-[10px] font-bold text-muted-foreground/60 uppercase mb-2 px-1 text-left">X-Service-ID</p>
                            <div className="flex items-center gap-2 p-4 bg-background rounded-xl border border-border-subtle text-left"><code className="text-xs font-mono text-primary flex-1 truncate text-left">{service?.id}</code><Button variant="ghost" size="icon" onClick={() => copyToClipboard(service?.id || "")}><Copy size={14} /></Button></div>
                        </CardContent>
                    </Card>
                </div>
            </div>
                    ...
                    {!generatedKey ? (
                        <>
                            <DialogHeader className="p-10 border-b border-border-subtle bg-background/30 text-center">
                                ...
                                <DialogTitle className="text-2xl font-bold italic uppercase tracking-tighter text-foreground text-center">Nova Conexão</DialogTitle>
                                    ...
                                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
                                        ...
                                        <div className="grid grid-cols-1 gap-6 text-left">
                                            <div className="space-y-2 text-left">
                                                <label className="text-[10px] font-bold text-muted-foreground uppercase px-1 text-left">Nome da Conexão</label>
                                                <Input 
                                                    value={formData.name}
