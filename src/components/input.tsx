    ...
    return (
        <div className={`flex flex-col gap-2 ${containerClassName}`}>
            <div className="flex flex-row justify-between items-center px-1">
                {label && <p className={`text-[10px] font-bold uppercase tracking-widest ${labelColor}`}>{label}</p>}
                {secondaryLabel && <div className={`text-[10px] font-bold uppercase tracking-widest ${secondaryLabelColor}`}>{secondaryLabel}</div>}
            </div>
            <input
                type={type}
